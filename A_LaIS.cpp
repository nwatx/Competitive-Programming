...
void solve() {
	int n; re(n);
	vi v(n); re(v);
	F0R(i, n) v[i]--; // avoid overflow of seg tree

	vi dp(n); // i = lais that finishes in v[i]
	Seg<int> seg; seg.init(n); // seg tree
	set<pi> S; // priority queue?
	S.insert({0, 0}); // base case
	F0R(i, n) {
		dp[i] = seg.query(0, v[i]) + 1; // queries biggest LAIS from from 0->v[i]
		seg.upd(v[i], dp[i]); // save the query
		while(sz(S) && v[i] >= S.begin()->f) { // while 
			seg.upd(S.begin()->f, S.begin()->s + 1);
			S.erase(bg(S)); // builds from small -> large in terms of v[i]
		}
		S.insert({v[i], dp[i]}); // set stores LAIS of ending in v[i]
	}
