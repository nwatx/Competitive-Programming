...
/*
3 2 4 4 1 3 2 1
either closed or open paired
query open pairs between a and b--that's the amount
...
void solve() {
	...
	F0R(i, 2*n) {
		...
		if(pre[x] == -1) {
			...
			cnt += abs(in.query(pre[x] + 1, i - 1) - out.query(pre[x] + 1, i - 1));
			out.upd(i, 1);
		}
		dbg(x, cnt);
	}
