...
void solve() {
    ...
    F0R(i, sz(s)) {
        // for each position we will try every possible substring
        F0R(j, n) {
            str x = v[j];
            // if a valid use is found
            bool canOverlap = true;
            F0R(k, i) if(dp[i] == MOD) canOverlap = false;

            vs toTest{x};

            if(canOverlap) R0F(k, sz(x) - 1) toTest.pb(x.substr(k));

            each(y, toTest) {
                if(i + sz(y) <= sz(s) && s.substr(i, sz(y)) == y) {
                    if(i == 0 || dp[i] + 1 < dp[i + sz(y)]) {
                        dp[i + sz(y)] = dp[i] + 1;
                        prev[i + sz(y)] = j;
                        pos[i] = i;
                    }
