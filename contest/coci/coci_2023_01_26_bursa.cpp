...
void search() {
	...
	// dbg(depth);
	// {dep, node, par}
	queue<pair<int, pi>> bfs;
	bfs.push({0, {0, -1}});
	while(sz(bfs)) {
		auto top = bfs.front(); bfs.pop();
		depth[top.s.f] = top.f;
		each(x, adj[top.s.f]) if(x != top.s.s && top.f < k && !blk.count(x)) bfs.push({top.f + 1, {x, top.s.f}});
	}
}
...
void solve() {
	...
	while(sz(bfs)) {
		auto top = bfs.front(); bfs.pop();
		d[top.s.f] = top.f;
		each(x, adj[top.s.f]) if(x != top.s.s) bfs.push({top.f + 1, {x, top.s.f}});
	}
