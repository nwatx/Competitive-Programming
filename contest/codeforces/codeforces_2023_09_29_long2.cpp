...
void solve() {
	...
	rep(q) {
		...
		int pRetMin = retMin, pRetMax = retMax;

		bool un = t == 0 ? dsu.unite(a, b) : dsu.unite(a, b + n);

		vi v(n);
		F0R(i, n) v[i] = dsu.get(i);
		dbg(v);
		...
		// both unites must be successful for something to change
		un &= (t == 0) ? dsu.unite(a + n, b + n) : dsu.unite(a + n, b);
		if(un) {
			if(t == 0) { // unite
				retMax--;
			} else { // disconnect
