...
- if different, find minimum rotations to make the same
- keep track of total, then use knapsack to find the best allocation
- you will get numbers x, y and we want to minimize \sum numbers >= x and y where they are not equal
*/
...
void gen() {
    // let dp[i][j][k] be the min cost for exactly (i, j) with maximal number k
    F0R(i, mx) F0R(j, mx) F0R(k, mx) dp[i][j][k] = INF;
    dp[0][0][0] = 0;
    FOR(k, 1, mx) { F0R(i, mx) F0R(j, mx) {
            if (dp[i][j][k - 1] == INF) continue;
            if (i + k < mx) {
                dp[i + k][j][k] = dp[i][j][k - 1] + (1LL << (k));
            }
            if (j + k < mx) {
                dp[i][j + k][k] = dp[i][j][k - 1] + (1LL << (k));
            }
            ckmin(dp[i][j][k], dp[i][j][k - 1]);
        }
    }
    F0R(i, mx) F0R(j, mx) {
        ans[i][j] = INF;
        FOR(k, 1, mx) ckmin(ans[i][j], dp[i][j][k]);
    }
}
...
void solve() {
    def(ll, x, y);
    dbg(x, y);
    pair<ll, pi> best{INF, {0, 0}};
    F0R(i, mx) F0R(j, mx) {
        if (x / (1LL << i) == y / (1LL << j)) {
            ckmin(best, {ans[i][j], {i, j}});
        }
    }
    dbg(best);
    ps(best.f);
