...
void solve() {
    int n; re(n);
    // proof that removing the diameter at least halves the size of each of the remaining trees
    V<unordered_set<int>> adj(n + 1);
    F0R(i, n - 1) {
        ints(a, b);
        adj[a].insert(b);
        adj[b].insert(a);
    ...
    FOR(i, 1, n + 1) remaining.ins(i);
    V<tuple<int, int, int>> ret;

    while(sz(remaining) > 0) {
        pi _t;
        vi par(n + 1);
        dbg(remaining);

        auto dfs = [&](const auto& dfs, int v, int p, pi dist) -> void {
            par[v] = p;
            dist.s = v;
            ckmax(_t, dist);
            // dbg(dist);
        ...
        each(head, remaining) {
            if (to_remove.count(head)) continue;
            dfs(dfs, head, 0, mp(0, 0));
            pi e1 = _t;
            _t = mp(0, 0);

            dfs(dfs, e1.s, 0, mp(0, 0));
            pi e2 = _t;
            _t = mp(0, 0);

            // reconstruct the nodes to remove
            vi path;
            int h = e2.s;
            while (h != 0) {
                path.pb(h);
                if (remaining.count(h)) {
                    dbg("before erase");
                    auto con = adj[h];
                    each(e, con) adj[e].erase(h);
                    adj[h].clear();
                    to_remove.insert(h);
                    dbg("post-erase");
                }
            ...
            dbg(e1, e2, path);
            int end1 = max(e1.s, e2.s);
            int end2 = min(e1.s, e2.s);
            ret.pb({e2.f + 1, end1, end2});
        }

        dbg(to_remove);
        each(e, to_remove) remaining.erase(e);
    }
