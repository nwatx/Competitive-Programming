...
void solve() {
	...
	// where dp[i][j] is i elements removed from lhs and j elements from rhs
	F0R(i, n) {
		F0R(j, n) {
			if(i + j > n) continue;
			// where dp[i][j][t] is the interval [i, j] remaining if player t is moving

			bool t = (i + j) % 2 == 0;
			// if t maximize
			if(t) {
				dp[i + 1][j] = dp[i][j] + v[i];
				ckmax(dp[i][j + 1], dp[i][j] + v[n - 1 - j]);
			} else {
				dp[i + 1][j] = dp[i][j] - v[i];
				ckmin(dp[i][j + 1], dp[i][j] - v[n - 1 - j]);
			}
		}
	}
