...
// if nothing from 0...max(a_i) works as k, then return -1
// narrowing possbilities of k => must be > max(b_i)
...
void solve_slow() {
    ...
    int lo = *max_element(all(b)) + 1;
    int hi = *max_element(all(a)) + 1;

    if (lo > hi) { ps(-1); return; }
