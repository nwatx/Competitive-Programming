...
void solve() {
    ...
    multiset<int> f; each(x, freq) f.ins(-x.s);
    ...
    F0R(i, m) {
        // for each cap find max under cap
        auto it = f.lower_bound(cap[i]);
        if(it == f.end()) it = prev(it);

        int actual = -(*it);
        dbg(actual);
        int diff = min(cap[i], actual);
        dbg(diff);

        f.erase(actual);
        if(cap[i] - diff > 0) f.ins(diff - cap[i]);
        ret += diff;
    }
