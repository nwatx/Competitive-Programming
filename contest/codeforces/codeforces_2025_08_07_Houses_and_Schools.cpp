...
void solve() {
    ...
    vl c(n); re(c);
    vl pfx(n + 1);
    F0R(i, n) pfx[i + 1] = pfx[i] + c[i];

    // let q[i] = k * c[k]
    // f[i][m] = min g[j][m - 1] + c_k (i - k)
    //         = min { g[j][m - 1] + i * c_k[j:i] - q[j:i] }
    //         = min_{j <= i} { g[j][m - 1] + i * (pfx[j] - pfx[i - 1]) }

    // g[i][m] = min f[j][m] + c_k (k - j)
    //         = min { f[j][m] + q[j:i] - j * c_k[j:i] }

    vl q(n + 1);
    F0R(i, n) q[i + 1] = (i + 1) * c[i];

    LineContainer lc_f, lc_g;
    lc_f.add(0, 0); lc_g.add(0, 0);
