...
void solve() {
	ll n, x; re(n, x);
	dbg(bitset<6>(n), bitset<6>(x));
	// check if bits are possible to change
	F0R(i, 64) {
		if((x & (1 << i)) && !(n & (1 << i))) {
			ps(-1);
			return;
	...
	// for each bit find first and last
	// for each bit, find last index that it is possible to make this change
