...
void solve() {
	// guaranteed that at least one row and one column exists with only one cell missing
    // then the sum must be row + a = col + b => you can choose arbitrary a
    // then you can go backwards from the end, e.g. if it's D then go left
    def(int, n, m);
    str s; re(s); s += "D";
    V<vl> a(n, vl(m)); re(a);
    // dbg(a);
    ...
    F0R(i, n) F0R(j, m) row[i] += a[i][j];
    F0R(i, m) F0R(j, n) col[i] += a[j][i];
