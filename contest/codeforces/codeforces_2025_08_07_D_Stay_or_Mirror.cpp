...
vl inversions(const vi &v) {
    vl ret(sz(v));
    F0R(i, sz(v)) {
        int c = 0;
        F0R(j, i) c += v[j] > v[i];
        ret[i] = c + (i > 0 ? ret[i - 1] : 0);
    }
    // dbg(v);
    return ret;
}

vl backwardinversions(vi v) {
    vl ret(sz(v));
    R0F(i, sz(v)) {
        FOR(j, i + 1, sz(v)) {
            if (v[j] < v[i]) ++ret[i];
...
void solve() {
    ...
    // calculate inversions from [0, i]
    // and calculate inversions from [i, n]
    vl a = inversions(p);
    a.insert(a.begin(), {0});

    vi pb = p;
    F0R(i, n) pb[i] = 2 * n - p[i];
    vl b = backwardinversions(pb);
    dbg(pb, b);
    b.pb(0);
    ...
    // a is min inversions from [0, i)
    // b is min inversions from [i, n) if reversed

    ll ret = min(a[n], b[0]);

    F0R(i, n) {
        ll bcost = min(b[i], n - i - 1);
        ckmin(ret, a[i] + bcost);
    }
