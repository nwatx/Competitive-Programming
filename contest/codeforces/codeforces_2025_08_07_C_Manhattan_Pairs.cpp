...
void solve() {
	...
	set<int> xa, xb;
	sor(v);
	F0R(i, sz(v) / 2) xa.ins(v[i].s);
	FOR(i, sz(v) / 2, n) xb.ins(v[i].s);
	sort(all(v), [](auto a, auto b) {
		return a.f.s < b.f.s;
	});

	set<int> ya, yb;
	F0R(i, sz(v) / 2) ya.ins(v[i].s);
	FOR(i, sz(v) / 2, n) yb.ins(v[i].s);

	set<int> q1, q2, q3, q4;
	F0R(i, n) if (xa.count(i) && ya.count(i)) q3.ins(i);
	F0R(i, n) if (xa.count(i) && yb.count(i)) q2.ins(i);
	F0R(i, n) if (xb.count(i) && ya.count(i)) q4.ins(i);
	F0R(i, n) if (xb.count(i) && yb.count(i)) q1.ins(i);
