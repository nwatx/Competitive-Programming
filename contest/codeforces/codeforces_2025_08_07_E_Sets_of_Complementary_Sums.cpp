...
void solve() {
    ...
    // dp[i] is # of ways with max(Q) = i
    // and the below condition is satisfied
    // (max(Q) + 1) * (|Q| - 1) - \sum{y} <= 0
    vi dp(x + 1);
    ...
    dbg(start - need);
    // then add {1} until this inequality becomes 0
    // then to decrease LHS by 1 we can just add 1 to everything start - need times

    // then the start is max(Q) = n + (start - need)
    int startQ = n + (start - need);
    // let's call this dp[0]

    // we can try adding all of the things from f(1) to f(n) which will gives us -2 => n - 2 for the LHS
    F0R(i, n - 1) {
        F0R(j, x) {
            if (j + (i + 1) <= x) {
                dp[j + i + 1] %= MOD;
