...
void solve() {
    ...
    vi a(n); re(a);
    sor(a);
    // now check if it is exactly one out of reach
    // i.e. can we recover
    bool flipped = false;
    if (a.back() - a.front() > k) {
        // then there must only be one instance of a back
        int a_back_freq = 0;
        each(x, a) a_back_freq += x == a.back();
        if ((a.back() - a.front() == k + 1) && a_back_freq == 1) {
            --a.back();
            flipped = true;
        } else {
            ps("Jerry");
    ...
    // then it's just even or odd counting
    int sum = 0;
    each(x, a) sum += x;
    bool tom_wins = sum % 2;
    tom_wins ^= flipped;
