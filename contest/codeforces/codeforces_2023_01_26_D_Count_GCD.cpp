...
void solve() {
	...
	// # of gcd(a, b) = c where we know a and c
	// lcm(a, b) = a * b / gcd(a, b)
	// lcm(a, b) = a * b / c
	// lcm(a, b) / a / b = 1 / c
	// a * b / lcm(a, b) = c
	// count number of b that divides lcm(a, b) / a
	...
	FOR(i, 1, n) {
		...
		if(c % v[i - 1]) ret[i] = 0;
			...
			ret[i] = ret[i - 1] * c / v[i - 1];
		}
	}

	ps(ret[n - 1]);

	// gcd is commutative so count the number such that it holds
	// count number of numbers <= m such that gcd(a_i - 1, num) = a_i
}
