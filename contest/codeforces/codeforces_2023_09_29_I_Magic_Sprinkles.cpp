...
void solve() {
	...
	auto update = [&](Pt p, int i) {
		// calculate angle
		db ang = 180 / PI * tanh(1.0 * p.y / p.x);
		if(p.x < 0) ang += 180;

		if(p.red) {
			red.ins({ang, i}); // find first blue less than this
			auto lb = blue.lb(ang);
			auto ub = blue.ub(ang);

			// count # of red in between
			db lang = (lb == blue.begin() || lb == blue.end()) ? -1 : *(--lb);
			db uang = ub == blue.end() ? 181 : *ub;

			dbg(lang, uang);
			int between = red.ook({uang, -2}) - red.ook({lang, MOD});
			ckmax(ret, between);
		} else {
			blue.ins(ang);
		}
	...
	for(int i = 0; i < n;) {
		int cdist = dist({v[i].x, v[i].y});
		while(i < n && dist({v[i].x, v[i].y}) == cdist) {
			update(v[i], i);
			i++;
		}
		// add everything
