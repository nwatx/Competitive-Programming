...
void solve() {
    ...
    rep(m2) {
        ...
        g2[a].pb(b);
        g2[b].pb(a);
        if (a < b) good2.ins({a, b});
        else good2.ins({b, a});
    }

    vpi to_erase;
    each(e, good) if (!good2.count(e)) to_erase.pb(e);
    each(p, to_erase) good.erase(p);

    vi safe(n + 1);
    each(p, good) { safe[p.f] = 1; safe[p.s] = 1; }

    V<vi> dp(n + 1, vi(n + 1, MOD));
    ...
    while(sz(pq)) {
        ...
        if (c != dp[u][v]) continue;
        each(u1, g1[u]) {
            each(u2, g2[v]) {
                auto cost = abs(u1 - u2) + dp[u][v];
                if (cost < dp[u1][u2]) {
                    dp[u1][u2] = cost;
                    pq.push({cost, u1, u2});
                }
