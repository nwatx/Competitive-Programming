...
void solve() {
    ...
    V<vi> pl(n + 1, vi(n + 1)), pr(n + 1, vi(n + 1));
    V<vi> mat(n, vi(n));
    ...
    // approach do prefix painting
    // dp[i][j - 1] += dp[i][j]
    // dp[i][j + 1] += dp[i][j]
    F0R(i, n) {
        vi row(n);
        F0R(j, n) row[j] = pl[i][j] + pr[i][j];
        partial_sum(all(row), begin(row));
        // dbg(row);
        F0R(j, n) row[j] += mat[i][j];
        dbg(row);
        
        F0R(j, n) {
            int total_paint = row[j];

            // then we need to remove
            if(total_paint & 1) {
                cnt++;

                // add another row of painting
                pl[i + 1][max(0, j - 1)]++;
                if(j < n - 1) pr[i + 1][min(n, j + 2)]--;
            }

            pl[i + 1][max(0, j - 1)] += pl[i][j];
            pr[i + 1][min(n, j + 1)] += pr[i][j];
        }
    }
