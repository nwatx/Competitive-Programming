...
void solve() {
	...
	auto good = [&](__float128 x) -> bool {
		__float128 curr = 0;
		V<__float128> diff(n);
		// eat the food expiring first
		multiset<pair<int, __float128>> food; // {expiry, qty}

		F0R(i, n) {
			food.insert({i + v[i].s, v[i].f});
			// get rid of expired food
			while(sz(food) && food.begin()->first <= i) food.erase(food.begin());

			__float128 need = x * k;
			// eat as much food as we can
			while(sz(food) && need > EPS) {
				auto top = food.begin(); food.erase(food.begin());
				__float128 diff = min(need, top->second);
				need -= diff;
				if(top->second - diff > EPS) {
					food.insert({top->first, top->second - diff});
				}
			}
	...
	__float128 l = 0, r = MOD;
	__float128 best = 0;

	for(int i = 0; i < 1500 && l + EPS < r; i++) {
		__float128 mid = l + (r - l) / 2;
		if(good(mid)) {
			l = mid;
		} else {
			r = mid;
	...
	if(l < EPS) cout << -1;
	else std::cout << setprecision(30) << (db)l;
}
