...
// if unlock
// explore
// then persist a window [l, r]

void solve() {
	int n; re(n);
	vl v(n); re(v);
	vl s(n + 1);
	F0R(i, n) s[i + 1] = s[i] + v[i];
	...
	// let dp[i][0] be the max score ending at i -- unlocked
	// let dp[i][1] be the max score ending at i -- locked

	// suppose we know a[i]
	// then dp[i...i + a[i]] = ckmax: dp[i] - a[i] + sum(i + 1, i + a[i])
	// we could also improve the existing max
	// for any unlocked end we can improve it by -2 * a[i] + sum(i + 1, i + a[i])
	...
	// 	if(i + x >= 0 && i + x < n) {
	// 		ckmax(dp[i + x], dp[i] - 2 * x + pfx[i + x + 1] - pfx[i]);
	// 	}
	// }
