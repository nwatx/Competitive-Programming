...
void solve() {
    ...
    Seg<mi> seg; seg.init(m + 1);
    F0R(i, n) {
        auto x = v[i];
        dbg(x);
        
        mi p_nothing = dp[x.f.f - 1] * seg.query(x.f.f, x.f.s);
        mi p_something = mi(1) - p_nothing;
        mi p_us = mi(v[i].s.f) / mi(v[i].s.s);

        dp[x.f.s] = p_nothing * p_us + dp[x.f.s] * (mi(1) - p_us);
        seg.upd(x.f.s, seg.query(x.f.s, x.f.s) * (mi(1) - p_us));
    }
