...
ll inversions(vi &v) {
	// inversions is # of elements where A_i < A_j but i > j
	// so count # of elements A_j s.t. A_j < A_i for i
	int n = sz(v);
	Tree<int> t;
	ll ret = 0;
	F0R(i, n) {
...
void solve() {
	...
	multiset<int> aa(all(a)), bb(all(b));
	if(aa != bb) {
		ps("NO");
		return;
	} 

	ll invA = inversions(a);
	ll invB = inversions(b);

	dbg(invA, invB);
	
	ps(((invA % 2) == (invB % 2)) ? "YES" : "NO");
}
