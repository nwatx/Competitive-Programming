...
void solve() {
    ...
    // find a cycle with one node with at least degree 4
    F0R(i, n) {
        if(deg[i] < 4) continue;
        fill(vis, vis + n, 0);
        ed.clear();
        ...
        if(good) {
            ...
            each(e, adj[i]) {
                if(cnt >= 2) break;
                if((ed.count({e, i}) || ed.count({i, e})) || e == i) continue;
                // if(in_cycle.count({i, e}) || in_cycle.count({e, i})) {
