...
void solve() {
	...
	for(int i = 0; i < n; i++) {
		for(int money = a; money >= 0; money--) {
			for(int cones = b; cones >= 0; cones--) {
				// bribe with cones
				if(money - C[i] + 1 >= 0 && cones - X[i] >= 0)
					ckmax(dp[i + 1][money - C[i] + 1][cones - X[i]], dp[i][money][cones] + P[i]);

				// normal purchase
				if(money - C[i] >= 0)
					ckmax(dp[i + 1][money - C[i]][cones], dp[i][money][cones] + P[i]);

				// no purchase
				ckmax(dp[i + 1][money][cones], dp[i][money][cones]);
			}
		}
	}
