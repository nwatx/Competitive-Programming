...
void solve() {
    ...
    rep(q) {
        ...
        int val = o == 1 ? a[x] : b[x];
        vi &search_vector = o == 1 ? sa : sb;
        int idx = upper_bound(all(search_vector), val) - search_vector.begin() - 1;
        int old = min(sa[idx], sb[idx]);
        if (o == 1) { ++a[x]; ++sa[idx]; }
        else { ++b[x]; ++sb[idx]; }
        int nw = min(sa[idx], sb[idx]);
        dbg(old, nw);
        ans = ans * nw / old;
        pr(ans, " ");
    }
