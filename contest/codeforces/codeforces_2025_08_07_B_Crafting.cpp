...
void solve() {
    ...
    vi diff(n);
    F0R(i, n) diff[i] = a[i] - b[i];

    int neg_diffs = 0;
    F0R(i, n) {
        if (diff[i] < 0) ++neg_diffs;
    }
    ...
    // ok, so we have one negative diff
    int idx = -1;
    F0R(i, n) if (diff[i] < 0) {
        assert(idx == -1);
        idx = i;
    ...
    int lo = MOD;
    F0R(i, n) if (i != idx) ckmin(lo, a[i] - b[i]); // can build at most a[i] - b[i] times
    if (lo >= abs(diff[idx])) ps("YES");
    else ps("NO");
}
