...
void solve() {
    ...
    string s; re(s);
    // sort by first time they appear
    set<int> unused;
    ...
    F0R(i, m) {
        ...
        for(auto it = unused.lower_bound(l); it != unused.end() && *it <= r; it++) {
            prio[*it] = pc++;
            to_erase.pb(*it);
        }

        each(x, to_erase) unused.erase(unused.find(x));
    }
    ...
    // all indices are now mapped to prioriites
    // binary search on # of operations
    rep(q) {
        int x; re(x); --x;
        dbg(x, prio[x]);
        // swaps == number of zeros there are [0, #1s]
        // if(!unused.count(x)) {
        if(!unused.count(x)) {
            if(seg.query(prio[x], prio[x])) {
                seg.upd(prio[x], 0);
                --oc;
            } else {
                seg.upd(prio[x], 1);
        ...
        // most possible 1s in the first pc spots
        int num = min(pc, oc) - seg.query(0, min(pc - 1, oc - 1));
        // {
        //     vi db;
        //     F0R(i, n) {
        //         db.pb(seg.query(i, i));
