...
void solve() {
	...
	vi diff;
	F0R(i, n - 1) diff.pb(v[i + 1] - v[i]);

	if(sz(diff) == 0) {
		ps(1); return;
	}

	ll g = sz(diff) >= 2 ? gcd(diff[0], diff[1]) : 1;
	FOR(i, 2, sz(diff)) {
		g = gcd(diff[i], g);
	}
	...
	// first positive integer congruent with gcd
	// v[0] (mod g)
	for(ll x = fpc(v[0], g), i = 0; i <= n + 1; x += g, i++) {
		if(!s.count(x)) {
			if(x < me) {
				ckmin(best, cnt + (me - x) / g);
			} else {
				// difference from highest
				ll diff = (x - me) / g;
				dbg(x, diff);
				ckmin(best, cnt + n * diff);
			}
		}
	}
