...
// want to get to a perfect square with at most one odd conversion
// 14 -> 8 
...
// 2 * 3 * 5 * 7
// reduce to 
// 2 * 3 * 5 by subtracting 6 * 2 * 3 * 5

// for any number we can reduce it to a singular prime
// then we get 2 * 3
// then we get 2 * 1 -> 

// for six we have 2 * 3
// so we reduce by 6 -> 3 -> 2 -> 1

void solve() {
    ll x; cin >> x;
    vi ret; ret.pb(x);
    F0R(k, 31) {
        if(x != p2(k) && x % p2(k + 1) == p2(k)) {
            x -= p2(k);
            ret.pb(x);
        }
    }
