...
char grid[3001][3001];
int deg[3001][3001];
bool vis[3001][3001], canRemove[3001][3001];

void rm(int x, int y) {
    // if the degree is below 2 remove it
    // dbg(x, y);
    // cerr << x << " " << y << '\n';
...
void solve() {
    ...
    F0R(i, r) F0R(j, c) {
        if(grid[i][j] == '^') canRemove[i][j] = false;
    ...
    F0R(i, r) F0R(j, c) {
        ...
        F0R(k, 4) {
            ...
            int ny = j + dy[k];
            if(nx < 0 || ny < 0 || nx >= r || ny >= c) continue;
            if(grid[nx][ny] == '^') deg[nx][ny]++;
        }
    }

    cerr << ("here") << '\n';
    // recursively remove things with degree < 2
    F0R(i, r) F0R(j, c) rm(i, j);

    bool good = true;
    F0R(i, r) F0R(j, c) if(grid[i][j] == '^' && deg[i][j] < 2) {
        good = false;
        break;
    }
