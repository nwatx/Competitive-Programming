...
template<class T> struct RMQ { // floor(log_2(x))
	...
	T query(int l, int r) { pi ret = v[index(l,r)]; ret.f = -ret.f; return ret; }
};
/* #endregion */
...
int dfs(int l, int r) {
	...
	int mid = rmq.query(l, r).s;
	int p1 = dfs(l, mid - 1), p2 = dfs(mid + 1, r);
	// count # of ways to satisfy
	for(int i = 1; i <= m; i++) {
		if(p1 != -1 && i == 1) dp[mid][1] = 0;
		else {
			// assert(i <= m);
			dp[mid][i] = (p1 >= 0 ? sum[p1][i - 1] : 1L) * (p2 >= 0 ? sum[p2][i] : 1L) % MOD;
		}
