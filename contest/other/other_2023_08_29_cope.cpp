...
void solve() {
	...
	F0R(i, n) {
		...
		// check if can run away directly
		// r < PI * r / v simplifies to 1 < PI / v == v < PI
		if(v < PI) { weights[i] = 0; }
		
		// you need to travel distance r - w, monster travels PI * r / v
        // r - w <= PI * r / v
		// w must be less than r / v
		// r - PI * r / v <= w
		// r * (1 - PI / v) <= w

        // r < PI * r / v + e
        // only works if e <= r
		// solve for e: r - e = pi * r => e = r - pi * r / v
		else {
			// rose has to travel
            db w = r * (1 - PI / v);
			dbg("rose travels:", r - w);
			dbg("monster travels:", PI * r);
			dbg(w);
			dbg(w);
			if(w * v > r) weights[i] = INF;
			else {
				weights[i] = ceil(w);
			}
		}
