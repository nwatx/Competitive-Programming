...
bool check(int cnt) {
    ...
    while (true) {
        ...
        // find next employee time
        int next = *m.begin();
        currTime = next;

        // dbg(currTime);
        
        auto toDel = m.upper_bound(next);
        // if(toDel != m.begin()) toDel = prev(toDel);
        // dbg("[deleted (exclusive)]", *toDel);
        m.erase(m.begin(), toDel);
        
        // dbg("[after del]", i, currTime, m);
        dbg(i, currTime, m);
    }
...
void solve() {
    ...
    assert(d >= 1 && d <= (int)1e9);
    F0R(i, n) cin >> A[i];
    ps(fstTrue(1, n, check));
}
