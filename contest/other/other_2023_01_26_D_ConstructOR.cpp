...
void solve() {
	ll a, b, d; re(a, b, d);
	// every time you find a 1 and 0 that doesn't satisfy you must multiply d by the missing spot
	// find the last 1 and 0 conflict and shift it right by that amount
	dbg(ts(bitset<16>(a)));
	dbg(ts(bitset<16>(b)));

	int mIdx = max(msbidx(a), msbidx(b));

	// d = 2 ^ mIdx * d

	// shift d right by the amount
	dbg(ts(bitset<32>(d)));
	d = d * ((1 << (mIdx + 1)) - 1);
	dbg(ts(bitset<32>(d)));

	// try to match d now
	// find the greaest one index
