...
void solve() {
    ...
    FOR(i, 1, 1001) {
        ...
        each(it, v) {
            // t + k(k') = i
            bool valid = (i) % it.k == 0 || it.t - i == 0;
            dbg(i, it.t, it.k, valid);
            if(valid) {
                if(it.t - i > 0) {
                    str add = "Connor, there are " + ts(it.t - i) + " minutes before the " + it.name + " is done cooking!";
                    if(sz(ret) == 0) ret = add;
                    else ret += " " + add;
                } else if(it.t - i == 0) {
                    string add = "Connor, the " + it.name + " is done!";
                    if(sz(ret) == 0) ret = add;
                    else ret += " " + add;
                }
            }
