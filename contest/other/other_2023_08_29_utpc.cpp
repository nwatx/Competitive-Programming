...
// problem: you know when you observe planets in front of your telescope
// you are given a time series let's say abcabca
// want to find the minimum length prefix that could explain it
// you can also get more observations
...
void solve() {
	...
	rep(n) {
		...
		if(t) {
			t_count++;
			assert(t_count * sz(hr.S) <= (int)2e5);
			FOR(s, 1, sz(hr.S) + 1) { // size to check, keep doubling
				bool good = true;
				int c_size = s;
				while(c_size <= sz(hr.S)) {
					int next = min(sz(hr.S), 2 * c_size);
					int size = next - c_size;
					// dbg(c_size, next, size);
					if(hr.hash(0, size - 1) == hr.hash(c_size, next - 1)) {
						c_size *= 2;
					} else {
						good = false;
						break;
