...
// there is at least two of everything and there is at most one odd (only if the string is odd)
// treat all the odd strings as one character
...
void solve() {
	...
	if(oC >= 2 || (oC == 1 && !(sz(s) & 1)) ) ps("NO SOLUTION");
	else {
		string ctn = "";
		string odd = "";
		each(e, f) {
			if(e.s & 1) F0R(i, e.s) odd += e.f;
			else F0R(i, e.s / 2) ctn += e.f;
		}
		string ret = ctn + odd;
		reverse(all(ctn));
		ret += ctn;
