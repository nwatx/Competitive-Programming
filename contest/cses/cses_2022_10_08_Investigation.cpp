...
void solve() {
    ...
    vl dist(n, INF), numMin(n), minFlights(n), maxFlights(n);
    pqg<pair<ll, int>> pq;
    dist[0] = 0;
    numMin[0] = 1;
    minFlights[0] = 0;
    ...
    while(sz(pq)) {
        ...
        each(x, adj[top.s]) {
            if(dist[top.s] + x.f < dist[x.s]) {
                pq.push({dist[x.s] = dist[top.s] + x.f, x.s});
                numMin[x.s] = numMin[top.s];
                minFlights[x.s] = minFlights[top.s] + 1;
                maxFlights[x.s] = maxFlights[top.s] + 1;
            } else if(dist[top.s] + x.f == dist[x.s]) {
                numMin[x.s] += numMin[top.s];
                ckmin(minFlights[x.s], minFlights[top.s] + 1);
                ckmax(maxFlights[x.s], maxFlights[top.s] + 1);
            }
            numMin[x.s] %= MOD;
        }
    ...
    ps(dist.back(), numMin.bk, minFlights.bk, maxFlights.bk);
}
