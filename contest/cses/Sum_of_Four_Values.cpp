...
int main() {
    ...
    R0F(i, n) {
        R0F(j, i) {
            ...
            if(hm.find(idx) != hm.end()) {
                ps(i + 1, j + 1, hm[idx].f + 1, hm[idx].s + 1);
                return 0;
            }
        }

        FOR(j, i + 1, n) if(v[i] + v[j] <= x) hm[v[i] + v[j]] = {i, j};
    }
    ...
    // map<pair<int, int>, vi> hm; // <<amount, frequency>, what we did to get to that number>
    // map<pair<int, int>, vi> dup;


    // F0R(i, n) { // check for each number does there exist some 3sum??
    //     // ps(v[i]);

    //     each(amt, hm) {
    //         // dbg(amt);
    //         if(amt.f.s >= 4) continue;
    //         int total = amt.f.f + v[i];
    //         if(total > x) continue;
    //         vi ret = amt.s;
    //         ret.pb(i);
    //         dup[{total, amt.f.s + 1}] = ret;
    //         if(total == x) break;
    //     }
