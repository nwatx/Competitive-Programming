...
int dist[21][21][21][21][4][4];
char A[21][21];

bool valid(int x, int y) {
	return !(x < 0 || y < 0 || x >= n || y >= n) && dist[x][y] == MOD;
}
...
int main() {
	...
	F0R(d1, 4) F0R(d2, 4) {
		fill(dist, dist + 21*21*21*21*4, MOD);
		queue<Pos> q;
		dist[0][0][0][0][d1][d2] = 0;
		q.push({0, 0, 0, 0, d1, d2});
		while(sz(q)) {
			Pos c = q.front();
			int cdist = dist[c.x1][c.y1][c.x2][c.y2];
			F0R(i, 4) F0R(j, 4) F0R(k, 4) {
				int nx1 = c.x1 + dx[k];
				int ny1 = c.y1 + dy[k];
				int nx2 = c.x2 + dx[k];
				int ny2 = c.y2 + dy[k];
