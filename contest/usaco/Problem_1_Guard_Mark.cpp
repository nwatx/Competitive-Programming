...
int ind[mx], h[mx], w[mx], s[mx];
ll dp[1<<20];

void solve() {
	int n, k; re(n, k);
	iota(ind, ind + n, 0);
	F0R(i, n) re(h[i], w[i], s[i]);
	fill(dp, dp + n, -1);
...
// the optimal stacking order?? w/ sorting
// the optimal stacking order would be
// resulting strength: s - h > s - h
