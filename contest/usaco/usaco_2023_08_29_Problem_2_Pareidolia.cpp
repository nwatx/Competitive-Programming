...
void solve() {
    ...
    V<vpi> dp(n + 1, vpi(sz(t), {-MOD, 0}));
    // first is number, second is cost
    // 0-'', 1-'b', 2-'be', ...
    // i is position
    // j is num
    dp[0][0] = {0, 0};
    ...
    F0R(i, n) {
        F0R(j, sz(t)) {
            ...
            ckmax(dp[i + 1][j], {dp[i][j].f, dp[i][j].s - (j > 0) * c[i]});

            if(t[j] == s[i]) {
                int k = (j + 1) % sz(t);
                ckmax(dp[i + 1][k], {dp[i][j].f + 1, dp[i][j].s});
            }
        }
    }

    pi ret = {-MOD, 0};
    F0R(i, sz(t)) ckmax(ret, {dp[n][i].f / sz(t), dp[n][i].s});
    ps(ret.f);
    ps(-ret.s);
}
