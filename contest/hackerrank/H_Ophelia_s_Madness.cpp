...
void solve() {
	int n; re(n);
	Seg<db> s1, s2; s1.init(n), s2.init(n);
	int cnt = 0;
	rep(n) {
		char c; re(c);
		if(c == 'R') {
			...
			// dbg(d);
			int x = cnt-1;
			db mu = s1.query(x-d+1,x)/d;
			db mu2 = s1.query(x-d+1, x-1)/d;
			// db sig = abs(s2.query(x - d + 1, x) - d*mu*mu)/(d-1);
			// db stddev = sqrt(sig);
			db stddev = abs((s1.query(x, x) - mu) * (s1.query(x, x) - mu2));
			stddev = sqrt(stddev);
			dbg(stddev);
			ps(mu - 2*stddev, mu + 2*stddev);
		}
	}
}
