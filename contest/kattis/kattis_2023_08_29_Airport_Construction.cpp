...
db abs(const P& p) { return sqrt(norm(p)); }
...
bool strict_isect(const Line& a, const Line& b) {
...
void solve() {
    ...
    F0R(i, n) {
        int j = (i + 1) % n;
        int k = (j + 1) % n;
        if(cross(poly[i], poly[j], poly[k]) > 0) {
            idx.pb(i);
            each(x, idx) {
                bad[x][k] = true;
                bad[k][x]=  true;
    ...
    F0R(i, n) FOR(j, i + 1, n) {
        ...
        F0R(k, n) {
            ...
            bool iS = strict_isect({poly[i], poly[j]}, {poly[k], poly[l]});

            // dbg("Checking intersection");
            // dbg(poly[i], poly[j]);
            // dbg(poly[k], poly[l]);
            // dbg(iS);
