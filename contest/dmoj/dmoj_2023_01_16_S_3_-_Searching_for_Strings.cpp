...
struct StringHashing {
    const ll A = (int)1e9 + 7, C = (int)1e9 + 9, B = 9973;
    vl hshA, hshB, pA, pB; int n;

    void init(str s) {
        n = sz(s);
        hshA.rsz(n); hshB.rsz(n);
        pA.rsz(n), pB.rsz(n);
        hshA[0] = s[0]; hshB[0] = s[0];

        FOR(i, 1, n) {
            hshA[i] = (hshA[i - 1] * B + s[i]) % A;
            hshB[i] = (hshB[i - 1] * B + s[i]) % C;
        }
    ...
    pair<ll, ll> hash(int l, int r) {
        if(l == 0) return {hshA[r], hshB[r]};
        return {((hshA[r] - hshA[l - 1] * pA[r - l + 1]) % A + A) % A,
				((hshB[r] - hshB[l - 1] * pB[r - l + 1]) % C + C) % C
		};
    }
};
...
void solve() {
    string a, b; re(a, b);
	set<pl> hsh;
	StringHashing hasher; hasher.init(b);
	vi fa(26), fb(26);
	each(x, a) fa[x - 'a']++;
	int ret = 0;
	F0R(i, sz(b)) {
		dbg(b[i]);
		fb[b[i] - 'a']++;
		if(i >= sz(a)) fb[b[i - sz(a)] - 'a']--;

		// check if frequencies are equal
		if(fa == fb) {
			dbg(i);
			pl hash = hasher.hash(i - sz(a) + 1, i);
